...
class VoiceService:
    ...
    def wake_status_text(self) -> str:
        labels = {
            ...
            "no_key": "Нужен ключ для облачного распознавания",
    ...
    def _capture_note_from_result(self, result: SpeechCaptureResult) -> str:
        ...
        if result.status == "no_speech":
            return "Не удалось получить текст. Проверьте микрофон или ключ для облачного распознавания."
    ...
    def _stt_note_from_result(self, result: TranscriptionResult) -> str:
        ...
        if result.status == "stt_key_missing":
            return "Нужен ключ для облачного распознавания."
        if result.status == "model_missing":
            return "Нужна локальная модель или ключ для облачного распознавания."
        if result.status == "no_speech":
            return "Не удалось получить текст. Проверьте микрофон или ключ для облачного распознавания."
